...
var metPlayButton = document.getElementById("metPlayButton");

metPlayButton.addEventListener("click", play);
...
function scheduleNote(beatNumber, time) {
  ...
  var osc = audioContextMet.createOscillator();
  var gainNode = audioContextMet.createGain();

  osc.connect(gainNode);
  gainNode.connect(audioContextMet.destination);
...
function scheduler() {
  while (nextNoteTime < audioContextMet.currentTime + scheduleAheadTime ) {
...
function play() {
  ...
  if (isPlaying) {
    currentTwelveletNote = 0;
    nextNoteTime = audioContextMet.currentTime;
...
function init(){
  audioContextMet = new (window.AudioContext || window.webkitAudioContext)();
